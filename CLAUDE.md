...
- `pacing.rs` — fakes audio-stream pacing for SAPI clients that expect sample-rate-throttled output (`SAMPLE_RATE_HZ`, `BYTES_PER_SAMPLE`); `marks.rs` handles bookmark/SSML mark plumbing back to `ISpTTSEngineSite`.
