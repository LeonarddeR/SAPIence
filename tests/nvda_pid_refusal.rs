...
use common::{CLSID_SAPIENCE_VOICE, DllHandle};
use core::ffi::c_void;
...
fn refuses_when_pid_matches_nvda() {
    ...
    let lib = unsafe { libloading::Library::new(common::dll_path()) }.expect("LoadLibrary failed");

    let set: libloading::Symbol<SetPidFn> = unsafe { lib.get(b"sapience_test_set_pid_override\0") }
        .expect("test hook missing — build with --features test-hooks");
    let clear: libloading::Symbol<ClearPidFn> =
        unsafe { lib.get(b"sapience_test_clear_pid_override\0") }.expect("clear hook missing");
    ...
    let hr = unsafe {
        (dll.get_class_object)(&CLSID_SAPIENCE_VOICE, &IClassFactory::IID, &mut factory_ptr)
    };
